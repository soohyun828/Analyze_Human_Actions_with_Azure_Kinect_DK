...
void print_body_information(k4abt_body_t body, FILE* fp)
{
    ...
    for (int i = 0; i < (int)K4ABT_JOINT_COUNT; i++)
    {
        ...
        fprintf(fp, "Joint[%d]: Position[mm] ( %f, %f, %f ); Orientation ( %f, %f, %f, %f); Confidence Level (%d) \n",
...
int main()
{
    ...
    do
    {
        ...
        {
            frame_count++;
            //fprintf(fp, "Start processing frame %d\n", frame_count);
            ...
            if (pop_frame_result == K4A_WAIT_RESULT_SUCCEEDED)
            {
                ...
                //fprintf(fp, "Time spent for this frame: %f s\n", time_spent);

                uint32_t num_bodies = k4abt_frame_get_num_bodies(body_frame);
                printf("%u bodies are detected!\n", num_bodies);
                //fprintf(fp,"%u bodies are detected!\n", num_bodies);
    ...
    //fprintf(fp,"Finished body tracking processing!\n");
